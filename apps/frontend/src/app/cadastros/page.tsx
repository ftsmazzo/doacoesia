...
export default function CadastrosPage() {
  ...
  const fetchDonorChecklist = useCallback(
    async (donorId: string) => {
      const response = await fetch(`${apiBase}/donors/${donorId}/documents/checklist`, {
        cache: "no-store",
      });
      if (!response.ok) throw new Error(await getErrorMessage(response, "Falha ao carregar checklist do doador."));
      setDonorChecklist((await response.json()) as DonorChecklist);
    },
    [apiBase],
  );
  ...
  useEffect(() => {
    if (!donationForm.donorId) return;
    void fetchDonorChecklist(donationForm.donorId).catch(() => undefined);
  }, [donationForm.donorId, fetchDonorChecklist]);
  ...
  async function onDonationSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage("Doacao criada. Agora envie a proposta detalhada.");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Erro ao cadastrar doação.");
    }
  }

  async function onUploadDonationProposal(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    ...
    try {
      ...
      setMessage("Proposta detalhada enviada com sucesso.");
    } catch (err) {
  ...
  async function uploadDonorDoc(type: DonorDocumentType, file: File | null) {
    if (!donationForm.donorId) {
      setError("Selecione um doador para enviar os documentos.");
      return;
    }
    if (!file) {
      setError("Selecione um arquivo para upload.");
    ...
    try {
      ...
      const response = await fetch(
        `${apiBase}/donors/${donationForm.donorId}/documents?type=${type}`,
        {
          method: "POST",
          body: formData,
        },
      );
      if (!response.ok) throw new Error(await getErrorMessage(response, "Falha no upload do documento do doador."));
      await fetchDonorChecklist(donationForm.donorId);
      setMessage("Documento do doador enviado com sucesso.");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Erro no upload.");
    } finally {
      setUploading(false);
  ...
  return (
    <AppShell
      title="Cadastros"
      subtitle="Ficha e identificacao no doador; proposta detalhada na doacao."
    >
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-4 sm:p-5">
        <h2 className="text-base font-semibold text-white">Checklist do doador</h2>
        <p className="mt-2 text-sm text-slate-300">Obrigatorios do doador: Ficha Anexo I e Documento de Identificacao.</p>
        <button
          type="button"
          onClick={() => void (donationForm.donorId ? fetchDonorChecklist(donationForm.donorId) : Promise.resolve())}
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {(Object.keys(donorDocLabels) as DonorDocumentType[]).map((type) => {
            const done = donorChecklist?.uploaded.includes(type) ?? false;
            return (
              <form
                key={type}
                ...
                onSubmit={(e) => {
                  ...
                  void uploadDonorDoc(type, input?.files?.[0] ?? null).then(() => {
                    if (input) input.value = "";
                  });
                }}
              >
                <p className="text-sm font-medium text-slate-100">{donorDocLabels[type]}</p>
                <p className={`mt-1 text-xs ${done ? "text-emerald-300" : "text-amber-300"}`}>{done ? "Enviado" : "Pendente"}</p>
                <input type="file" className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-800 px-2 py-1.5 text-xs text-slate-100" />
                <button disabled={uploading} type="submit" className="mt-2 rounded-lg bg-cyan-600 px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-40">
                  {uploading && uploadingType === type ? "Enviando..." : "Enviar / Reenviar"}
                </button>
              </form>
            );
          })}
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-4 sm:p-5">
        <h2 className="text-base font-semibold text-white">Proposta da doacao</h2>
        <p className="mt-2 text-sm text-slate-300">Obrigatorio da doacao: Proposta Detalhada.</p>
        <form className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center" onSubmit={onUploadDonationProposal}>
        ...
        {!createdDonationId ? <p className="mt-3 text-xs text-slate-400">Crie uma doacao antes de anexar a proposta.</p> : null}
